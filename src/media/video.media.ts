...
export class VideoMedia extends Media {
    ...
    public getPlaybackStatus(): PlaybackStatus {
        return new VideoPlaybackStatus(this.mediaElement.status,
    ...
    public setTrack(type: EnumTrack, trackId: string, enabled: boolean): EnumError {
        ...
        let i: number = 0;
        if ((status === EnumMediaStatus.BUFFERING) ||
    ...
    public setVolume(level: number): EnumError {
        let status = this.getStatus();
        if ((status !== EnumMediaStatus.PLAYING) && (status !== EnumMediaStatus.PAUSED)) {
            return EnumError.PLAYER_INVALID_STATE;
        }
    ...
    public pause(): EnumError {
        let status = this.getStatus();
        if ((status !== EnumMediaStatus.BUFFERING) && (status !== EnumMediaStatus.PLAYING)) {
            return EnumError.PLAYER_INVALID_STATE;
        }
